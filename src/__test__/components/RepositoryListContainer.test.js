  ...
  describe("RepositoryListContainer", () => {
    it("renders repository information correctly", () => {
      ...
      getAllByTestId("repoStars").forEach((item, index) => {
        expect(item).toHaveTextContent(repositories.edges[index].node.stargazersCount);
      });

      getAllByTestId("repoForks").forEach((item, index) => {
        expect(item).toHaveTextContent(repositories.edges[index].node.forksCount);
      });
      ...
      getAllByTestId("repoReviews").forEach((item, index) => {
        expect(item).toHaveTextContent(repositories.edges[index].node.reviewCount);
      });
    });
  });
});
